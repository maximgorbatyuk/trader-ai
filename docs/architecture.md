...
Stock splits and reverse merges change the denomination of holdings, issued supply, cost basis, and compatible issuer orders while preserving capitalization as closely as integer quantities allow. Participant orders are cleared so the book can reform around the new denomination. Primary issuance proceeds settle into a company-owned cash balance. Dividends are based on company capitalization and owned shares, with stability checks, a configured ceiling, and issuer cash as the final funding limit. Corporate cash has its own append-only ledger and never mixes with participant balances or bank revenue. See [Corporate cash](logic/corporate-cash.md).

...
The model remains the final authority over each order's side, company, exact limit price, quantity, and reason, and over one optional whole-share direct company investment. The snapshot exposes eligible minimum and maximum share quantities alongside current exposure, residual executable asks, the shared automated-buy envelope, active price bounds, price-time priority ceilings, and structured feedback from the previous application. The coordinator reacquires the market lock, applies valid owner-requested cancellations first, revalidates and applies the investment second, rebuilds the live market and exposure context, and then accepts or rejects each exact order without silently clamping it. Rejection feedback carries a stable category and relevant corrective bounds while preserving the model's original request.

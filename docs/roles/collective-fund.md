...
- An active fund buys and sells automatically, but normally keeps 10% of its worth as a cash buffer for member deposits. From the trading day before any member becomes leave-eligible, and while a departure remains possible, its automated buying uses a 15% buffer instead.
- A fund uses the same normalized company direction as a rule-based Individual: momentum, order flow, industry sentiment, the effective audit, and current fundamentals. Its inherited risk profile controls the balance between defensive quality and forecast growth, while its temperament changes activity and waiting. The signal produces buy, sell, and wait probabilities rather than a certain action; see [Participant rules](../participant-rules.md).
- After an action is selected, ordinary fund buy and sell limits draw symmetrically above or below the active reference and remain inside the executable band. The Individual and AI Agent automated-buy exposure policy does not apply to fund sizing. Cash-raising and liquidation sells are clamped into the executable band so they can cross. See [Share price formation](../rules/share-price-formation.md) and [LULD price controls](../rules/luld.md).
...
- When a fund sells holdings for a gain over their cost basis, its founder immediately receives a configurable share of that realized gain, paid out of the fund's cash so the rest of the gain stays in the fund. A sale at a loss or break-even pays nothing.
...
- A fund passes part of its own dividend income through to members, divided by deposit size, and withholds some management fee from each member's share that stays in the fund's cash.
...
- At the start of each trading day, a fund snapshots a voluntary-departure quota equal to 15% of its members, rounded up so even a small fund can release one member. Once that quota has been used, any other member wanting out waits for the next trading day. The limit covers both an ordinary leave and a switch to another fund, but not administrative removals — capacity enforcement, fund closure, and a member leaving the market.

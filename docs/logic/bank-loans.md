...
- A trader may borrow up to a fixed fraction of its total worth (cash plus holdings) when an explicit loan workflow requests borrowing. Every active trader type can carry a term loan, including the human player and collective funds.
...
- Each cycle the loan is charged a scheduled principal repayment plus interest, paid from the borrower's available, unreserved cash and recorded on the trader's cash movements.
...
- Repayment clears assessed fees first, then overdue interest, overdue principal, and finally current principal. Paying overdue principal reduces both its overdue classification and outstanding principal by the same amount.

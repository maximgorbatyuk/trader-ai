...
- An Individual crosses the best residual in-band ask when one is available; otherwise it can place only a bounded in-band passive bid. Demand already allocated earlier in the same decision batch retains price-time priority, preventing a later generated bid from jumping ahead of it.

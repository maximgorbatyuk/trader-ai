...
public sealed class AiTradingOptions
{
    ...
    public const string DefaultSystemPromptTemplate =
        ...
        - Financials and audits are advisory evidence. The management forecasts can be wrong. No indicator directly moves the market price, and price direction remains probabilistic.
        - Short selling remains unavailable; do not model a sell as a negative holding or an uncovered order.
        - Price every order with priceOffsetPercent, a signed percentage the backend applies to the company's currentPrice at execution time: limitPrice = currentPrice x (1 + priceOffsetPercent / 100). It resolves against the freshest price when your decision is applied, which is later than this snapshot, so express intent as an offset rather than an absolute price that may have gone stale. A negative percent rests below the current price and a positive percent above it: to buy patiently use a small negative percent and to cross a seller a small positive one, while to sell patiently use a positive percent and to exit quickly a negative one.
        - You retain authority over each priceOffsetPercent and quantity. The backend resolves your percent to a limit price and clamps it onto the allowed band, so a modest offset is never rejected for drifting out of range while this request ran; a buyEnvelope is safe at its orderPrice, and if your offset resolves to another price inside the active and allowed bounds, its quantity limits are recomputed at that resolved price before acceptance. Read every absolute price field such as orderPrice, maximumPrioritySafeBuyPrice, and executable sell prices as a reference and convert it to a percent relative to currentPrice.
        - When maximumPrioritySafeBuyPrice is present, choose a buy priceOffsetPercent whose resolved price does not exceed that ceiling: existing demand has priority over supply at it. A buyEnvelope may guide a passive bid at the priority ceiling when crossing a higher residual sell would be unsafe. A missing buyEnvelope means no buy currently satisfies the exact market, exposure, and priority constraints.
        ...
        - Use the best executable sell price and buyEnvelope to deploy capital deliberately. When exposure is Below and an executable sell exists, a buy must cross that seller - pick a priceOffsetPercent whose resolved price reaches the best executable sell - rather than rest unrealistically.

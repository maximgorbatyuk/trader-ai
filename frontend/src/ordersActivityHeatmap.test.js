...
test('groups the latest three trading days into six rows and twenty-one columns', async () => {
  ...
  assert.equal(heatmap?.rows[0][1].observedCycleEnd, 6)
  assert.equal(heatmap?.rows[0][1].isPartial, true)
...
test('derives cell boundaries from the supplied trading-day length', async () => {
  ...
  assert.equal(heatmap?.rows[0][15].observedCycleEnd, 3)
  assert.equal(heatmap?.rows[0][15].isPartial, true)
})

test('describes a partial dividend cell without dropping either state', async () => {
  const { ordersActivityCellLabel } = await loadHeatmap()
  ...
  assert.equal(
    ...
    'Day 16, cycles 26–30: 98 orders, in progress through cycle 28, dividend paid',
  )
})

test('moves keyboard focus within the six-by-twenty-one heatmap', async () => {
  const { heatmapPositionAfterKey } = await loadHeatmap()

  assert.deepEqual(heatmapPositionAfterKey?.(2, 10, 'ArrowRight'), { row: 2, column: 11 })
  assert.deepEqual(heatmapPositionAfterKey?.(2, 10, 'ArrowLeft'), { row: 2, column: 9 })
  assert.deepEqual(heatmapPositionAfterKey?.(2, 10, 'ArrowDown'), { row: 3, column: 10 })
  assert.deepEqual(heatmapPositionAfterKey?.(2, 10, 'ArrowUp'), { row: 1, column: 10 })
  assert.deepEqual(heatmapPositionAfterKey?.(2, 10, 'Home'), { row: 2, column: 0 })
  assert.deepEqual(heatmapPositionAfterKey?.(2, 10, 'End'), { row: 2, column: 20 })
  assert.deepEqual(heatmapPositionAfterKey?.(0, 0, 'ArrowUp'), { row: 0, column: 0 })
  assert.deepEqual(heatmapPositionAfterKey?.(5, 20, 'ArrowRight'), { row: 5, column: 20 })
  assert.equal(heatmapPositionAfterKey?.(2, 10, 'Enter'), null)
})
